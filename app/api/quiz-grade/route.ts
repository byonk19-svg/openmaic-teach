...
Do not combine multiple missing elements into one multi-part question. Choose ONE missing rubric element for this revision, mention only that omission in feedback, and ask for that element alone. When several deficiencies are present, target the dominant reasoning error rather than selecting the first omitted rubric element mechanically. If the supplied rubric includes a feedback-priority policy, follow that policy. For an entirely vague answer without a rubric-specific priority, ask the learner to choose one unresolved question and identify specific evidence that would help answer it. Other missing elements can be addressed in later revisions.
